...
export async function run(env: RunnerEnv): Promise<RunnerResult> {
  ...
  if (!exists(cardPath)) {
    ...
    return { exitCode: 1, markdown: "", outputWritten: false, commentPosted: false, reason: "card-path not found" };
  }

  let card: AgentCard;
  try {
    card = JSON.parse(read(cardPath)) as AgentCard;
  } catch (e) {
    write(`::error::could not parse AgentCard at "${cardPath}" — ${(e as Error).message}`);
    return { exitCode: 1, markdown: "", outputWritten: false, commentPosted: false, reason: "malformed AgentCard JSON" };
  }

  const markdown = generateReadme(card, { hideBadges, anchorPrefix });
  ...
  if (outputPath) {
    try {
      ...
      write(`::error::could not write output-path "${outputPath}" — ${(e as Error).message}`);
  ...
  if (wantsComment) {
    if (!token) {
      ...
      if (!issueNumber) {
        ...
        const body = `### AgentCard README — \`${cardPath}\`\n\n${markdown}\n\n_Generated by [agent-card-readme-generator-action](https://github.com/mizcausevic-dev/agent-card-readme-generator-action)._`;
