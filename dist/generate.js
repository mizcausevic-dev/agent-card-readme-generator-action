...
function header(card, opts) {
    ...
    if (!opts.hideBadges) {
        ...
        if (card.safety_posture?.incident_response_uri)
            badges.push("✅ incident response URI");
        else
            badges.push("⚠ no incident response URI");
        lines.push("");
        lines.push(badges.join("  ·  "));
    }
    lines.push("");
    lines.push(`**Agent id:** \`${card.agent.id}\` · **Version:** \`${card.agent.version}\` · **Provider:** ${card.agent.provider}`);
    if (card.agent.homepage)
        lines.push(`**Homepage:** ${card.agent.homepage}`);
    return lines.join("\n");
}
...
function models(card) {
    ...
    lines.push(`|---|---|`);
    for (const m of card.capabilities.models_used)
        lines.push(`| \`${m.model}\` | ${m.role ?? "—"} |`);
    return lines.join("\n");
}
function tools(card, anchorPrefix) {
    const lines = [`## Tools (${card.capabilities.tools.length})`, ""];
...
function refusals(card) {
    const cats = card.refusal_taxonomy ?? [];
    const lines = [`## Refusal taxonomy (${cats.length})`, ""];
    if (cats.length === 0) {
    ...
    for (const r of cats) {
        ...
        if (r.example_prompts && r.example_prompts.length > 0) {
            lines.push("");
            for (const ex of r.example_prompts)
                lines.push(`- "${ex}"`);
        }
        lines.push("");
    }
...
function safetyPosture(card) {
    const s = card.safety_posture ?? {};
    const lines = [`## Safety posture`, ""];
...
export function generateReadme(card, opts = {}) {
    if (!card || !card.agent || !card.capabilities)
        throw new Error("input must be a valid AgentCard");
